...
# Show src/ changes only (skip .polygon/, docs, chore)
...
**Voice**: technical reference doc. Short, clear, plain. Not a story, not a sales pitch. Style guide: `.polygon/spec/docs-site/docs/style-guide.md` → "Changelog / Release Notes Voice".
...
**Manifest `changelog` field** (terminal display during `polygon update`): same rules, single string with `\n` separators, group with `**Enhancements:**` / `**Bug Fixes:**` / `**Internal:**` bold prefixes.
...
| `notes` | Brief guidance for users (e.g., "run `polygon update --migrate` to sync"). Shown inline in terminal during update. |
...
| `reason` | **Why** the user might see this entry flagged as modified. Version-specific context (e.g. "Polygon 0.4.0 skipped hashing this path — pristine copies show as modified. [r] is safe."). Keeps version-specific hints out of `update.ts`. | optional |
...
- `rename` uses the **project-local** `.polygon/.template-hashes.json` (auto-maintained by Polygon), NOT the manifest's `allowed_hashes` field.
  ...
  "migrations": [
    {
      ...
      "to": ".claude/skills/polygon-new-path/SKILL.md",
...
Full reference: `.polygon/spec/docs-site/docs/release-lifecycle.md`.
...
- **`allowed_hashes`** — required ONLY for `safe-file-delete`. `rename` classification uses project-local `.polygon/.template-hashes.json`; you do NOT need to collect historical hashes for rename entries.
...
Watch for:
- **Orphan files** — stale paths written by the old version that don't match any rename/safe-file-delete. Grep `find . -path "*/skills/*" -not -path "*/polygon-*"` to catch plain-name skill dirs.
...
- **Backup bloat** — confirm `.polygon/.backup-*/` doesn't contain `/worktrees/` or `/workspace/` paths.
