---
name: polygon-spec-bootstrap
description: "Bootstrap project-specific Polygon coding specs with a platform-neutral single-agent workflow. Use when creating or refreshing .polygon/spec guidelines, analyzing a codebase with GitNexus, ABCoder, or source inspection, decomposing package/layer spec work, and writing real codebase-backed spec docs without placeholder text."
---

# Polygon Spec Bootstrap

Use this skill to create or refresh `.polygon/spec/` guidelines from the real codebase. One capable agent owns the full loop: analyze the repository, choose the spec boundaries, write the docs, and verify the result. The workflow does not depend on a specific host, CLI, or agent brand.

## Workflow

1. Confirm Polygon is initialized and inspect the current `.polygon/spec/` tree.
...
| Writing high-signal Polygon spec files | [references/spec-writing.md](references/spec-writing.md) |
...
- Do not leave placeholder text, empty headings, or copied boilerplate in `.polygon/spec/`.

## Done Criteria

- `.polygon/spec/` describes the project as it exists now.
