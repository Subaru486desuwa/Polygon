...
Polygon connects the same local architecture to different AI tools. `.polygon/` stores the shared runtime; platform directories store adapter files that define how each AI tool enters Polygon.

When a local AI modifies Polygon, it should distinguish two file categories first:

- **Shared files**: `.polygon/workflow.md`, `.polygon/tasks/`, `.polygon/spec/`, `.polygon/scripts/`.
...
| agents | `.claude/agents/`, `.codex/agents/`, `.kiro/agents/` | Define `polygon-research`, `polygon-implement`, and `polygon-check`. |
...
- session-start injection of a `.polygon/` overview.
...
To change behavior, inspect platform workflows/skills/commands and `.polygon/workflow.md`.
...
1. Read `.polygon/workflow.md` to confirm the shared flow.
...
5. If the change affects the shared flow, synchronize `.polygon/workflow.md` or `.polygon/spec/`.

Do not modify only platform files and forget the shared workflow. Do not modify only `.polygon/workflow.md` and forget that platform entry points may still contain old descriptions.
