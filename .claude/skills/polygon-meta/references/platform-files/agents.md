...
| --- | --- |
| `polygon-research` | Investigate the question and write findings into the current task's `research/`. |
| `polygon-implement` | Implement against `prd.md`, `info.md`, `implement.jsonl`, and related spec/research. |
| `polygon-check` | Review changes, fix discovered issues, and run necessary checks. |
...
| --- | --- |
| Implement agent must follow extra restrictions | The platform's `polygon-implement` agent file. |
| Check agent must run project-specific commands | `polygon-check` agent file, and `.polygon/spec/` if needed. |
| Research agent must output a fixed format | `polygon-research` agent file. |
