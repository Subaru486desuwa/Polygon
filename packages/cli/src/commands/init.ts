...
export function requireSupportedPython(command: string): string {
  ...
  if (!versionOutput) {
    throw new Error(
      `Python command "${command}" not found. Polygon init requires Python ≥ 3.9.`,
    );
  }

  if (!isSupportedPythonVersion(versionOutput)) {
    throw new Error(
      `${versionOutput} detected via "${command}", but Polygon init requires Python ≥ 3.9.`,
    );
  }
...
function logPythonAdaptationNotice(command: string): void {
  ...
  console.log(
    chalk.blue(
      `📌 ${osName} detected: Polygon rendered Python commands as "${command}" in generated hooks, settings, and help text`,
    ),
  );
}
...
"Welcome to Polygon! Your init just set me up to help you fill the project
...
function getJoinerTaskJson(developer: string, taskName: string): TaskJson {
  ...
  return emptyTaskJson({
    id: taskName,
    name: taskName,
    title: `Joining: Onboard to this Polygon project (${developer})`,
    description:
      "Onboard a new developer to an existing Polygon project: learn the workflow, conventions, and find assigned work",
    status: "in_progress",
    dev_type: "docs",
    priority: "P1",
    creator: developer,
...
### 1. What Polygon is + the workflow

Polygon is a workflow layer over Claude Code / Cursor / etc. that keeps AI
...
/**
 * Create joiner onboarding task for a new developer on an existing Polygon
...
async function handleReinit(
    ...
    const { action } = await inquirer.prompt<{ action: string }>([
      {
        type: "list",
        name: "action",
        message: "Polygon is already initialized. What would you like to do?",
        choices: [
  ...
  if (doAddDeveloper) {
    ...
    // joiner onboarding onto an existing Polygon project.
...
export async function init(options: InitOptions): Promise<void> {
  ...
  const banner = figlet.textSync("Polygon", { font: "Rebel" });
  console.log(chalk.cyan(`\n${banner.trimEnd()}`));
  console.log(
    chalk.gray(
      "\n   Slim agent workflow — persistent tasks, specs & multi-LLM activity logs\n",
    ),
  );
          ...
          for (const pkg of detected) {
            const { specSource } = await inquirer.prompt<{
            ...
            }>([
              {
                ...
                choices: [
                  { name: "From scratch (Polygon default)", value: "blank" },
