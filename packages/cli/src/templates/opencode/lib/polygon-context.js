...
// Debug logging
const DEBUG_LOG = "/tmp/polygon-plugin-debug.log"
...
// Matches `polygon-implement`, `polygon-check`, `polygon-research` exactly.
// Used by chat.message plugins to skip injection inside Polygon sub-agent turns.
const POLYGON_SUBAGENT_RE = /^polygon-(implement|check|research)$/
 ...
 */
export function isPolygonSubagent(input) {
  if (!input || typeof input !== "object") return false
  const agent = typeof input.agent === "string" ? input.agent.trim() : ""
  return POLYGON_SUBAGENT_RE.test(agent)
}

/**
 * Polygon Context Manager
 */
export class PolygonContext {
  constructor(directory) {
    this.directory = directory
    debugLog("context", "PolygonContext initialized", { directory })
  }
  ...
  isPolygonProject() {
    return existsSync(join(this.directory, ".polygon"))
  }

  getContextKey(platformInput = null) {
    const override = stringValue(process.env.POLYGON_CONTEXT_ID)
    if (override) {
  ...
  readContext(contextKey) {
    try {
      const contextPath = join(this.directory, ".polygon", ".runtime", "sessions", `${contextKey}.json`)
  ...
  _resolveSingleSessionFallback() {
    const sessionsDir = join(this.directory, ".polygon", ".runtime", "sessions")
  ...
  resolveTaskDir(taskRef) {
    ...
    return join(this.directory, ".polygon", "tasks", normalized)
  }
