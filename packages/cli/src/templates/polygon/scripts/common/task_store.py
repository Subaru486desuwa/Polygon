...
_SEED_EXAMPLE = (
    ...
    "Run `python3 .polygon/scripts/get_context.py --mode packages` to list available specs. "
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    # Agent curates real entries in Phase 1.3 (see .polygon/workflow.md).
    # Agent-less platforms (Kilo / Antigravity / Windsurf) skip this — they
    # load specs via the polygon-before-dev skill instead of JSONL.
    seeded_jsonl = False
    ...
    if seeded_jsonl:
        print(
            ...
            "check.jsonl — see .polygon/workflow.md Phase 1.3",
            file=sys.stderr,
        )
...
def _auto_commit_archive(
    ...
    ``git add -f .polygon/`` (which would fan out to caches/backups)
    and points users at ``session_auto_commit: false``.

    Honors ``session_auto_commit`` in ``.polygon/config.yaml``: when
