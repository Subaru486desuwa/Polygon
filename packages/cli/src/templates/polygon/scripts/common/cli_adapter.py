...
class CLIAdapter:
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "polygon-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'polygon-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'polygon', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/polygon-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Windsurf uses workflow directory: .windsurf/workflows/polygon-<name>.md
            Copilot uses prompt files: .github/prompts/<name>.prompt.md
            Pi uses prompt templates: .pi/prompts/polygon-<name>.md
            Claude/OpenCode use subdirectory: .claude/commands/polygon/<name>.md
        """
        if self.platform == "pi":
            ...
            if len(parts) >= 2 and parts[0] == "polygon":
                ...
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return prompts_dir / f"polygon-{filename}.md"
        ...
        if self.platform == "windsurf":
            ...
            if len(parts) >= 2 and parts[0] == "polygon":
                filename = parts[-1]
                return workflow_dir / f"polygon-{filename}"
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "polygon":
            # Convert polygon/<name>.md to polygon-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"polygon-{filename}"
            )

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_polygon_command_path(self, name: str) -> str:
        """Get relative path to a polygon command file.
        ...
        Note:
            Cursor: .cursor/commands/polygon-<name>.md
            Codex: .agents/skills/polygon-<name>/SKILL.md
            Kiro: .kiro/skills/polygon-<name>/SKILL.md
            Gemini: .gemini/commands/polygon/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Windsurf: .windsurf/workflows/polygon-<name>.md
            Pi: .pi/prompts/polygon-<name>.md
            Others: .{platform}/commands/polygon/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/polygon-{name}.md"
        elif self.platform == "codex":
            # 0.5.0-beta.0 renamed all skill dirs to add the `polygon-` prefix
            # (see that release's manifest for the 60+ rename entries).
            return f".agents/skills/polygon-{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/polygon-{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/polygon/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        elif self.platform == "windsurf":
            return f".windsurf/workflows/polygon-{name}.md"
        elif self.platform == "kilo":
            ...
            return f"{self.config_dir_name}/commands/polygon/{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    ...
    Detection order:
    1. POLYGON_PLATFORM environment variable (if set)
    ...
    env_platform = os.environ.get("POLYGON_PLATFORM", "").lower()
    if env_platform in (
        "claude",
        "opencode",
        "cursor",
    ...
    # (.agents/skills/polygon-* dirs) and no explicit platform config dir.
    ...
    if agents_skills.is_dir() and not _has_other_platform_dir(
        ...
        try:
            for entry in agents_skills.iterdir():
                if entry.is_dir() and entry.name.startswith("polygon-"):
                    return "codex"
        except OSError:
            pass
