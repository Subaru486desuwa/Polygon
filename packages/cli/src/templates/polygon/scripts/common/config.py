...
Reads settings from .polygon/config.yaml with sensible defaults.
"""
...
def _strip_inline_comment(value: str) -> str:
    ...
    Mirrors :func:`common.polygon_config._strip_inline_comment` so both
...
def get_session_auto_commit(repo_root: Path | None = None) -> bool:
    ...
    Set ``session_auto_commit: false`` in ``.polygon/config.yaml`` to skip
...
def get_spec_base(package: str | None = None, repo_root: Path | None = None) -> str:
    """Get the spec directory base path relative to .polygon/.
