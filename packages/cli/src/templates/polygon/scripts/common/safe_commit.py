...
----------------------
A real user incident: a project's `.gitignore` listed `.polygon/` (company-wide
...
`git add -f .polygon/` — which fan-out-included every ignored subtree
(`.polygon/.backup-*/`, `.polygon/worktrees/`, `.polygon/.template-hashes.json`,
`.polygon/.runtime/`), committing 548 files / 83474 lines of caches/backups.

Design
------
- Scripts only stage SPECIFIC product paths (journal files, index.md, the
  current task dir, the archive dir). Never the whole `.polygon/` tree.
- If plain `git add <specific>` fails with "ignored by", DO NOT retry with
  ``-f``. The presence of `.polygon/` in `.gitignore` is treated as user
  intent ("keep .polygon/ local-only"). The script warns and skips the
...
- The warning includes a negative example: ``Do NOT use `git add -f .polygon/` ...``
...
# Paths under .polygon/ that must NEVER be auto-staged. Listed here so the
# warning to the user can show concrete subpaths to ignore individually
# instead of ignoring the whole `.polygon/` tree.
POLYGON_IGNORED_SUBPATHS = (
    ".polygon/.backup-*",
    ".polygon/worktrees/",
    ".polygon/.template-hashes.json",
    ".polygon/.runtime/",
    ".polygon/.cache/",
)


def safe_polygon_paths_to_add(repo_root: Path) -> list[str]:
    ...
    Included:
      - .polygon/workspace/<developer>/journal-*.md
      - .polygon/workspace/<developer>/index.md
      - .polygon/tasks/<task-dir>/   (every active task directory)
      - .polygon/tasks/archive/      (whole archive subtree, if present)

    Excluded (intentionally — these must not be staged):
      - .polygon/.backup-*, .polygon/worktrees/,
        .polygon/.template-hashes.json, .polygon/.runtime/, .polygon/.cache/
    """
    paths: list[str] = []
...
def safe_archive_paths_to_add(
    ...
    `.polygon/tasks/` subtree the old way (active tasks + archive). New
...
def print_gitignore_warning(paths: list[str]) -> None:
    ...
    ``Do NOT use `git add -f .polygon/``` — agents reading the warning are
    known to invent that command, which fans out to ignored caches/backups.
    """
    print(
        "[WARN] git add failed because .polygon/ paths are ignored by your .gitignore.",
        file=sys.stderr,
    )
    print(
    ...
    if paths:
        ...
        print(
            "[WARN]   .polygon/workspace/<developer>/{journal-*.md,index.md}",
            file=sys.stderr,
        )
        print(
            "[WARN]   .polygon/tasks/<task-dir>/",
            file=sys.stderr,
        )
        print(
            "[WARN]   .polygon/tasks/archive/",
            file=sys.stderr,
        )
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Recommended: change your .gitignore from `.polygon/` to specific",
        file=sys.stderr,
    )
    print(
    ...
    for sub in POLYGON_IGNORED_SUBPATHS:
        print(f"[WARN]   {sub}", file=sys.stderr)
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Or, if you intentionally keep .polygon/ local-only, set in",
        file=sys.stderr,
    )
    print(
        "[WARN] .polygon/config.yaml:",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Do NOT use `git add -f .polygon/` — it pulls in backups, worktrees,",
        file=sys.stderr,
    )
    print(
