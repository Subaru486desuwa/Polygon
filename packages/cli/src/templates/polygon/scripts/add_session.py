...
def _auto_commit_workspace(repo_root: Path) -> None:
    ...
    `.polygon/` tree, and if `.gitignore` blocks the specific paths we
    warn + skip — never retry with ``-f``.

    Honors ``session_auto_commit`` in ``.polygon/config.yaml``: when set to
    ...
    paths = safe_polygon_paths_to_add(repo_root)
    if not paths:
