...
Polygon workflow skills usually share one semantic set: brainstorm, before-dev, check, update-spec, break-loop. Multi-file built-in skills such as `polygon-meta` use layered references.
...
They should not store task state; task state belongs in `.polygon/tasks/` and `.polygon/.runtime/`.
...
1. **Keep entry files short; references carry long content**. This matters especially for multi-file skills like `polygon-meta`.
...
4. **Put project-specific capabilities in local skills**. Do not put team-private flows into public `polygon-meta`.

If the user only wants local AI to know one more project rule, usually create a project-local skill or update `.polygon/spec/` instead of changing a Polygon built-in workflow skill.
