...
3. `.polygon/scripts/common/active_task.py`
4. `.polygon/scripts/common/session_context.py`
5. `.polygon/workflow.md`
...
3. Read the hook script and identify inputs, outputs, and called `.polygon/scripts/`.
4. Modify hook behavior.
5. If the hook depends on workflow content, synchronize `.polygon/workflow.md`.
...
If the hook ultimately calls `.polygon/scripts/get_context.py` or `session_context.py`, editing the local script is usually more robust than hard-coding content in the hook.
...
- Hooks should read project-local `.polygon/`; they should not depend on Polygon upstream source paths.
