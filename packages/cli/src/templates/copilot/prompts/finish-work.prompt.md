---
description: "Polygon Copilot prompt: Finish Work — survey + archive task + record session journal"
---
...
Filter out paths under `.polygon/workspace/` and `.polygon/tasks/` — those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this prompt's own work.
