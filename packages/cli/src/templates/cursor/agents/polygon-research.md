---
name: polygon-research
description: Polygon research agent. Use this exact agent for Polygon task research and research/ persistence. Do not use generic/default/generalPurpose agents for Polygon research.
...
Run `python3 ./.polygon/scripts/task.py current --source` → active task path. If no active task is set, ask the user where to write output; do NOT guess.
...
- Code files (`src/`, `lib/`, …)
- Spec files (`.polygon/spec/`) — main agent should use `update-spec` skill instead
- `.polygon/scripts/`, `.polygon/workflow.md`, platform config (`.claude/`, `.cursor/`, etc.)
...
- `.polygon/spec/xxx.md` — <description>
