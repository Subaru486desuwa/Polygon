 ...
 *   no .polygon/                        → creator bootstrap task
 *   .polygon/ exists, .developer missing → joiner onboarding task
...
describe("init() joiner onboarding", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "polygon-joiner-int-"));
  ...
  /**
   * Helper: simulate a fresh clone of an existing Polygon project — `.polygon/`
  ...
  /** Helper: simulate same-dev re-init — both `.polygon/` and `.developer` exist */
  ...
  it("#2 existing .polygon/ + no .developer → joiner onboarding task created", async () => {
    simulateExistingCheckout();
    ...
    expect(prd).toContain(".polygon/spec/");
    ...
    expect(prd).toContain(
      `${expectedPythonCmd} ./.polygon/scripts/task.py list --assignee bob`,
    );
    expect(prd).toContain(`${expectedPythonCmd} ./.polygon/scripts/task.py finish`);
    expect(prd).toContain(
      `${expectedPythonCmd} ./.polygon/scripts/task.py archive 00-join-bob`,
    );
  ...
  it("#2b issue #204: existing .polygon/ but tasks/ empty → bootstrap fallback (--yes alone, no --force)", async () => {
    // Mirrors the exact reproduction in issue #204: first run aborted partway
    // after writing the .polygon/ skeleton but before creating bootstrap;
  ...
  it("#3 existing .polygon/ + .developer → no task created", async () => {
  ...
  it("#6 joiner creation failure surfaces as warning, init does not crash", async () => {
    ...
    // thwarted by making .polygon/tasks read-only right before dispatch,
  ...
  // Tests #7/#8 cover the handleReinit path — the default flow when .polygon/
  ...
  it("#7 handleReinit path: existing .polygon/ + no .developer → joiner task created", async () => {
    simulateExistingCheckout();
  ...
  it("#8 handleReinit path: existing .polygon/ + .developer → no task created", async () => {
