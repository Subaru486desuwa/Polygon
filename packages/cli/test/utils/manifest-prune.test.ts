...
describe("pruneOrphanManifestKeys", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "polygon-prune-"));
  ...
  it("preserves every .polygon/* entry regardless of platform-collect output", () => {
    const hashes = {
      ".polygon/workflow.md": "h1",
      ".polygon/scripts/task.py": "h2",
      ".polygon/config.yaml": "h3",
    };
  ...
  it("does NOT rewrite disk when nothing was pruned", () => {
    const hashes = { ".polygon/workflow.md": "h1" };
    saveHashes(tmpDir, hashes);

    const hashFile = path.join(tmpDir, ".polygon", ".template-hashes.json");
...
describe("isCwdHomedir / homedir guard helpers", () => {
  ...
  it("homedirBypassEnabled reflects POLYGON_ALLOW_HOMEDIR env var", () => {
    const orig = process.env.POLYGON_ALLOW_HOMEDIR;
    try {
      ...
      for (const value of ["0", "false", "true", ""]) {
        process.env.POLYGON_ALLOW_HOMEDIR = value;
        expect(homedirBypassEnabled()).toBe(false);
      }
    } finally {
      if (orig === undefined) delete process.env.POLYGON_ALLOW_HOMEDIR;
      else process.env.POLYGON_ALLOW_HOMEDIR = orig;
    }
  });
  ...
  it("homedirGuardMessage mentions the command and the bypass env var", () => {
    ...
    expect(msgInit).toContain("POLYGON_ALLOW_HOMEDIR=1");

    const msgUninstall = homedirGuardMessage("uninstall");
    expect(msgUninstall).toContain("uninstall");
    expect(msgUninstall).toContain("POLYGON_ALLOW_HOMEDIR=1");
  });
});
