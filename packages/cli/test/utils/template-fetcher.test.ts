...
describe("getInstallPath", () => {
  it("returns spec path for 'spec' type", () => {
    ...
    expect(result).toBe(path.join("/project", ".polygon/spec"));
  });
  ...
  it("works with different cwd values", () => {
    ...
    expect(result).toBe(path.join("/home/user/my-project", ".polygon/spec"));
  });
});
...
async function withGitRegistry<T>(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "polygon-git-reg-"));
  ...
  try {
    ...
    execFileSync(
      ...
      [
        ...
        "-c",
        "user.email=polygon@example.com",
        "commit",
        "-m",
        "fixture",
      ],
...
async function withFakeGit<T>(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "polygon-fake-git-"));
  ...
  it("downloads marketplace templates through the same Git backend", async () => {
    await withGitRegistry(
      ...
      async ({ registry, tmpDir }) => {
        ...
        expect(
          fs.readFileSync(
            path.join(cwd, ".polygon", "spec", "rules.md"),
            "utf-8",
          ),
        ).toBe("remote rules\n");
      },
  ...
  it("uses direct download mode when index.json is absent", async () => {
    await withGitRegistry(
      ...
      async ({ registry, tmpDir }) => {
        ...
        const cwd = path.join(tmpDir, "project");
        const specDir = path.join(cwd, ".polygon", "spec");
