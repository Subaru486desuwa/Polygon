...
4. **Capture learnings** — write new knowledge back to `.polygon/spec/` when a task produces any
...
On first use: `python3 ./.polygon/scripts/init_developer.py <your-name>` — creates `.polygon/.developer` (gitignored) + `.polygon/workspace/<your-name>/`.

### Spec System

`.polygon/spec/` holds coding guidelines. Start from `.polygon/spec/<package>/<layer>/index.md` and `.polygon/spec/guides/index.md`; update when a new pattern, pitfall, or technical decision is worth keeping.

### Task System

Every task lives under `.polygon/tasks/{MM-DD-name}/` with `prd.md`, `task.json`, optional `research/`, `info.md` (plus `implement.jsonl` / `check.jsonl` seeds used only for sub-agent dispatch).

```bash
python3 ./.polygon/scripts/task.py create "<title>" [--slug <name>]   # status=planning
python3 ./.polygon/scripts/task.py start <name>          # status=in_progress, sets active pointer
python3 ./.polygon/scripts/task.py current --source      # show active task
python3 ./.polygon/scripts/task.py finish                # clear active pointer
python3 ./.polygon/scripts/task.py archive <name>        # move to archive/{year-month}/
python3 ./.polygon/scripts/task.py list [--mine] [--status <s>]
python3 ./.polygon/scripts/task.py create-pr [name] [--dry-run]
```

> `task.py --help` has the authoritative list. Session pointers live under `.polygon/.runtime/sessions/`.
...
```bash
python3 ./.polygon/scripts/task.py activity-append [<name>] --action <verb> [--note "<text>"]
python3 ./.polygon/scripts/task.py activity-log [<name>]    # readable timeline
```
...
- **Resolution** — `platform` auto-derives from `POLYGON_CONTEXT_ID`; `model` is best-effort (pass `--model` or set `POLYGON_ACTIVITY_MODEL`, else null).
...
Session records under `.polygon/workspace/<developer>/journal-N.md` (auto-rotates at 2000 lines):

```bash
python3 ./.polygon/scripts/add_session.py --title "Title" --commit "hash" --summary "Summary"
```

### Context Script

```bash
python3 ./.polygon/scripts/get_context.py                            # full session runtime
python3 ./.polygon/scripts/get_context.py --mode phase --step <X.Y>  # detailed guide for a workflow step
```
...
- Multi-turn implementation work, or anything worth a persistent record → `python3 ./.polygon/scripts/task.py create "<title>"`, brainstorm into prd.md, then `task.py start <task-dir>`.
...
Iterate on prd.md with the user (the `polygon-brainstorm` skill has the template); persist research to `{task_dir}/research/` — files survive compaction, chat doesn't.
...
Iterate on prd.md with the user. Ultracode is on: when research splits into independent angles, fan out parallel `polygon-research` agents via the Workflow tool, each dispatch prompt starting with `Active task: <task path from \`task.py current\`>`; trivial planning turns need no fan-out.
...
Implement in the main session by default — you hold the full conversation context. Read `{task_dir}/prd.md` + `research/` and the relevant `.polygon/spec/` guides before coding; run lint / type-check / tests before reporting done.
Sub-agents (`polygon-implement` / `polygon-check` / `polygon-research` via the Task/Agent tool; `polygon-check` and `polygon-update-spec` also exist as skills) are optional tools for parallelizable or context-heavy chunks — not a required pipeline. **Sub-agent dispatch protocol (all platforms, all sub-agents)**: the dispatch prompt MUST start with `Active task: <task path from \`task.py current\`>` — for `polygon-research` this resolves which `{task_dir}/research/` to write into. Sub-agents never run git commit/push/merge; if you ARE one, work directly — don't spawn another.
Then commit (Phase 3.4): the main session states the commit plan, lists unrecognized dirty files for the user to include/exclude (never silently commit them), runs `git commit` — no `--amend`, no push. Wrap up with `/polygon:finish-work` (it refuses to run on a dirty working tree).
...
Main session edits code directly (inline mode — no polygon-implement / polygon-check sub-agents). Read `{task_dir}/prd.md` + `research/` and relevant `.polygon/spec/` guides before coding; lint / type-check / tests before reporting done.
Then commit (Phase 3.4): state the commit plan, list unrecognized dirty files for the user to include/exclude, run `git commit` — no `--amend`, no push. Wrap up with `/polygon:finish-work` (it refuses to run on a dirty working tree).
[/workflow-state:in_progress-inline]
...
Ultracode is on: for real verification, fan out `polygon-check` agents via the Workflow tool — one per review dimension (see the "Polygon × Ultracode" section for the pipeline template) — and reconcile findings in the main session. Implement stays single by default; fan out `polygon-implement` only when the prd splits into independent, non-overlapping units (worktree isolation, experimental). Every dispatch prompt starts with `Active task: <task path from \`task.py current\`>`, repo-root relative; Workflow agents self-load prd + jsonl (no hook injection), and check fan-out runs at repo root, not in a worktree. Workflow agents never run `git commit` / `push` / `merge`.
**Sub-agent self-exemption**: if you are already a polygon-implement / polygon-check worker, do the work directly — no further fan-out.
After checks reconcile green, the MAIN session drives the commit (Phase 3.4) — commit plan in user-facing text, unrecognized dirty files confirmed with the user, `git commit`, no `--amend`, no push — then `/polygon:finish-work`. If implement fan-out used worktrees, merge them back into the task branch first.
[/workflow-state:in_progress-ultra]

### Phase 3: Finish
- 3.1 Final verification
- 3.2 Debug retrospective `[on demand]` (`polygon-break-loop`)
- 3.3 Spec update `[when new knowledge surfaced]`
- 3.4 Commit changes
- 3.5 `/polygon:finish-work`

[workflow-state:completed]
Code committed; run `/polygon:finish-work` to wrap up (archive the task + record session).
...
| User intent | Route |
|---|---|
| Wants a new feature / requirement unclear | `polygon-brainstorm` |
| About to write code / start implementing | `polygon-before-dev` (then implement directly in the main session) |
| Finished writing / want to verify | `polygon-check` |
| Stuck / fixed same bug several times | `polygon-break-loop` |
| Spec needs update | `polygon-update-spec` |

### Loading Step Detail

```bash
python3 ./.polygon/scripts/get_context.py --mode phase --step <step>   # e.g. --step 1.1
```
...
Research in the main session with any available tool (code search, web, MCP), or fan out `polygon-research` sub-agents for independent angles. Either way, findings go to `{task_dir}/research/<topic>.md` — one file per topic, with file:line citations and version constraints. Research left only in chat is lost on compaction.

#### 1.3 Configure jsonl context `[sub-agent dispatch only · once]`

Only needed when Phase 2 will dispatch `polygon-implement` / `polygon-check` sub-agents: curate `{task_dir}/implement.jsonl` and `check.jsonl` (one JSON object per line: `{"file": "<path>", "reason": "<why>"}`) with the spec + research files those agents need. Working inline — the default — skip this step.

#### 1.4 Activate task

```bash
python3 ./.polygon/scripts/task.py start <task-dir>    # status → in_progress
```
...
Default: implement directly in the main session. Read `prd.md`, consult `research/`, load `polygon-before-dev` for spec context if the area is unfamiliar. Dispatch a `polygon-implement` sub-agent only when it genuinely helps (parallelizable chunk, context-heavy isolation); its prompt must start with `Active task: <task path>` and state that it must not spawn further sub-agents.

#### 2.2 Verify `[repeatable]`

Run project lint / type-check / tests. For substantive changes, load the `polygon-check` skill (or dispatch the `polygon-check` sub-agent) to review spec compliance and cross-layer consistency. Fix → re-check until green.
...
If the same issue was fixed multiple times this task, load `polygon-break-loop`: classify the root cause, explain why earlier fixes failed, capture prevention into spec.

#### 3.3 Spec update `[when new knowledge surfaced]`

If this task produced knowledge worth keeping — a new pattern or convention, a pitfall you hit, a technical decision — capture it under `.polygon/spec/` (the `polygon-update-spec` skill has the format). Nothing new? Move on; no judgment ceremony needed.
...
Remind the user they can run `/polygon:finish-work` (archives the task, records the session).
...
1. **Config-persistent** — `.polygon/config.yaml` sets `ultracode.enabled: true` → the hook serves `planning-ultra` / `in_progress-ultra` breadcrumbs (claude only).
...
| Phase | Default | Notes |
|---|---|---|
| 1.2 Research | **fan-out** | One `polygon-research` agent per independent angle; each writes its own `{task_dir}/research/*.md`. |
...
| 2.2 / 3.1 Check | **fan-out** | One `polygon-check` agent per review dimension; main session reconciles. |
...
```javascript
const TASK = ".polygon/tasks/06-08-example";   // repo-root-relative task dir
...
const outs = [];
for (const d of dims) {
  outs.push(agent(ACTIVE + "You are polygon-check, " + d.body, { agentType: "polygon-check" }));
}
const results = [];
