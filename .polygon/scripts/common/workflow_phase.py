...
Extracts step-level content from .polygon/workflow.md and optionally filters
...
def get_phase_index() -> str:
    ...
    `## Phase 2: Execute`, `## Phase 3: Finish`, stops at the
    `## Customizing` section (the docs footer).
    ...
    for i, line in enumerate(lines):
        ...
        if start is None and stripped == _PHASE_INDEX_HEADING:
            start = i
            continue
        if start is not None and stripped.startswith("## Customizing"):
            end = i
            break
...
def resolve_effective_platform(platform: str, config: dict) -> str:
    ...
    or ``"codex-sub-agent"`` based on ``.polygon/config.yaml`` ``codex.dispatch_mode``.
