...
Resolution rules:
- platform/session are AUTO-resolved from the hook-injected `POLYGON_CONTEXT_ID`
...
- model is best-effort only: Claude Code hooks do NOT expose the model name to
  ...
  `POLYGON_ACTIVITY_MODEL` env var, else null. platform is the reliable field.
"""
...
def resolve_actor(
    ...
    platform: explicit arg > `POLYGON_CONTEXT_ID` prefix > `AI_AGENT` env > "unknown".
    session:  explicit arg > the context key's session, but ONLY when the
              ...
              the ambient `POLYGON_CONTEXT_ID` session belongs to Claude, not
              Codex — so session is left None rather than mislabeling it.
    model:    explicit arg > `POLYGON_ACTIVITY_MODEL` env > None (hooks don't
