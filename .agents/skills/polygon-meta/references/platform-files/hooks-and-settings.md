...
- workflow-state hook: parses `[workflow-state:STATUS]` blocks from `.polygon/workflow.md` and emits the body matching the current task `status` on each user input. Parser-only; the script does not embed fallback content.
...
| Pi Agent | `.pi/settings.json`, `.pi/extensions/polygon/` |

Whether these files exist in a project depends on which `polygon init --<platform>` flags the user ran.
...
| `inject-workflow-state.py` | Parses `[workflow-state:STATUS]` blocks in `.polygon/workflow.md` and emits the body matching the current task status. Falls back to `Refer to workflow.md for current step.` when no matching block exists. |
...
| Per-turn hint policy should change | `[workflow-state:STATUS]` block in `.polygon/workflow.md`. The hook parses workflow.md verbatim — no script edit required. |
...
3. **Hooks read local `.polygon/`, not upstream source**. `.polygon/scripts/` and `.polygon/workflow.md` in the user project are the default targets.
...
3. Manually run the `.polygon/scripts/get_context.py` or `task.py current --source` command that the hook depends on.
4. Check whether active task state exists in `.polygon/.runtime/sessions/`.
