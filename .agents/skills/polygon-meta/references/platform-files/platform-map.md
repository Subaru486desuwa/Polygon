...
This page lists common Polygon file locations in a user project by platform. Whether a platform directory exists in an actual project depends on which `polygon init --<platform>` commands the user ran.
...
| Pi Agent | `--pi` | `.pi/` | `.pi/skills/` | `.pi/agents/` | `.pi/extensions/polygon/` + `.pi/settings.json` |

## Capability Groups

### Polygon Sub-Agent Support

These platforms usually have `polygon-research`, `polygon-implement`, and `polygon-check` files:
...
4. User wants project rules: prefer `.polygon/spec/` or a project-local skill.
5. User wants Polygon behavior: edit `.polygon/workflow.md` plus platform hooks/agents/skills/commands.
