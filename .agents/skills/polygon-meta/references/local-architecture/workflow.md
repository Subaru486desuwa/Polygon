...
`.polygon/workflow.md` is the Polygon workflow source of truth inside the user project. An AI does not need Polygon source code to understand how the current project should move tasks forward; this file is enough.

## File Responsibilities

`.polygon/workflow.md` has three responsibilities:
...
- Platforms with sub-agent support: dispatch `polygon-implement` by default for implementation and `polygon-check` for checking.
- Platforms without sub-agent support: the main session reads skills such as `polygon-before-dev`, then executes directly.
...
After editing, make the AI reread `.polygon/workflow.md`; do not assume the flow from the old conversation is still valid.
...
- skills, such as `polygon-brainstorm` and `polygon-check`.
