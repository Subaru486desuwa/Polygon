...
`.polygon/spec/` is the user's project-specific engineering spec library. Polygon is not about making AI memorize conventions; it injects relevant specs or requires the AI to read them at the right time.
...
`.polygon/config.yaml` can declare packages:
...
```jsonl
{"file": ".polygon/spec/cli/backend/index.md", "reason": "CLI backend conventions"}
{"file": ".polygon/spec/cli/unit-test/conventions.md", "reason": "Test expectations"}
```
...
When the AI learns a new rule during implementation or debugging, it should update `.polygon/spec/` rather than only summarizing it in chat.

## Local Customization Points

| Need | Edit location |
| --- | --- |
| Add a new spec layer | `.polygon/spec/<package>/<layer>/index.md` and corresponding guideline files. |
| Change monorepo spec mapping | `packages` / `default_package` / `spec_scope` in `.polygon/config.yaml`. |
...
| Change when specs should be updated | Phase 3.3 in `.polygon/workflow.md` and the `polygon-update-spec` skill. |

## Boundaries

`.polygon/spec/` is the user's project specification, not a permanent copy of Polygon built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating Polygon default templates as immutable documents.
