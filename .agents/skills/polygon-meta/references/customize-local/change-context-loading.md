...
2. `.polygon/scripts/get_context.py`
3. `.polygon/scripts/common/session_context.py`
4. `.polygon/scripts/common/task_context.py`
5. `.polygon/scripts/common/active_task.py`
...
| Source | Purpose |
| --- | --- |
| `.polygon/workflow.md` | Workflow and next-action hints. |
| `.polygon/tasks/<task>/prd.md` | Current task requirements. |
| `.polygon/tasks/<task>/implement.jsonl` | Spec/research to read before implementation. |
| `.polygon/tasks/<task>/check.jsonl` | Spec/research to read during checking. |
| `.polygon/spec/` | Project specs. |
| `.polygon/workspace/` | Session records. |
...
| Change hints on each user input | `[workflow-state:STATUS]` block in `.polygon/workflow.md`. The `inject-workflow-state` hook is parser-only and reads the block verbatim. |
...
```jsonl
{"file": ".polygon/spec/backend/index.md", "reason": "Backend conventions"}
{"file": ".polygon/tasks/04-28-x/research/api.md", "reason": "API research"}
```
...
- `.polygon/scripts/common/session_context.py`
...
- agent pull: edit the read steps in the corresponding `polygon-implement` / `polygon-check` agent file.
...
python3 ./.polygon/scripts/task.py list-context <task>
