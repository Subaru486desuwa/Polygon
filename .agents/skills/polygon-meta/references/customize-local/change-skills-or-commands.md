...
4. Whether project rules already exist in `.polygon/spec/`
...
| Team project conventions | Prefer `.polygon/spec/` or a project-local skill. |
| Change Polygon flow semantics | Synchronize `.polygon/workflow.md`. |
...
- Which `.polygon/` files to read.
- Which scripts to run.
- How to report after completion.

If a command only repeats workflow rules, prefer making it reference/read `.polygon/workflow.md` instead of maintaining a second copy of the flow.
...
```text
.claude/skills/project-polygon-local/
└── SKILL.md
```
...
- Do not hide long-term engineering conventions inside a command; write them to `.polygon/spec/`.
